...
TTEditorDefines = {
	TOOLBAR: {
		INSERTION: {
			CONTAINER: {
				ID: 'TTEditor-Panel-Insertion-Container',
				ATTRS: {
					'class': 'TTEditor-Panel-Insertion-Container'
				}
			},
			CONTENT: {
				ID: 'TTEditor-Panel-Insertion-Content',
				ATTRS: {
					'class': 'TTEditor-Panel-Insertion-Content uk-inline',
				}
			},
			PLUSBUTTON: {
				ID: 'TTEditor-Panel-Insertion-Button',
				ATTRS: {
					'class': 'TTEditor-Panel-Insertion-Button',
					'type': 'button',

				}
			},
			ITEMS: {
				ID: 'TTEditor-Panel-Insertion-Items',
				ATTRS: {
					'class': 'TTEditor-Panel-Insertion-Items',
...
TTEditorMethods = {
	...
	/* create left insertion panel, properties in TTEditorDefines.TOOLBAR.INSERTION */
	generateInsertionPanel: function () {
		var panelContainer	= $('<div></div>');
		panelContainer.attr('id', TTEditorDefines.TOOLBAR.INSERTION.CONTAINER.ID);
		Object.keys(TTEditorDefines.TOOLBAR.INSERTION.CONTAINER.ATTRS).forEach(function (attrName) {
			panelContainer.attr(attrName, TTEditorDefines.TOOLBAR.INSERTION.CONTAINER.ATTRS[attrName]);
		});

		var insertContent 	= $('<div></div>');
		insertContent.attr('id', TTEditorDefines.TOOLBAR.INSERTION.CONTENT.ID);
		Object.keys(TTEditorDefines.TOOLBAR.INSERTION.CONTENT.ATTRS).forEach(function (attrName) {
			insertContent.attr(attrName, TTEditorDefines.TOOLBAR.INSERTION.CONTENT.ATTRS[attrName]);
		});
		panelContainer.append(insertContent);

		var insertButton	= $('<button><span uk-icon="icon: plus"></span></button>');
		insertButton.attr('id', TTEditorDefines.TOOLBAR.INSERTION.PLUSBUTTON.ID);
		Object.keys(TTEditorDefines.TOOLBAR.INSERTION.PLUSBUTTON.ATTRS).forEach(function (attrName) {
			insertButton.attr(attrName, TTEditorDefines.TOOLBAR.INSERTION.PLUSBUTTON.ATTRS[attrName]);
		});
		insertContent.append(insertButton);

		var insertItems	= $('<div></div>');
		insertItems.attr('id', TTEditorDefines.TOOLBAR.INSERTION.ITEMS.ID);
		Object.keys(TTEditorDefines.TOOLBAR.INSERTION.ITEMS.ATTRS).forEach(function (attrName) {
			insertItems.attr(attrName, TTEditorDefines.TOOLBAR.INSERTION.ITEMS.ATTRS[attrName]);
		});
		insertContent.append(insertItems);

		return panelContainer[0];
	},

	/* check current browser is mobile or PC */
	isMobile: function () {
		var check = false;
		(function(a) {
...
TTEditor.prototype.init 		= function () {
	...
	if (!Props.ttOptions.leftInsertButton) {
...
},{"./additionals/constants.js":1,"./additionals/methods.js":2,"./additionals/utils.js":3,"./extensions/array.js":4,"codemirror":11,"codemirror/addon/display/panel.js":6,"codemirror/addon/display/placeholder.js":7,"codemirror/addon/mode/overlay.js":8,"codemirror/addon/selection/active-line.js":9,"codemirror/addon/selection/mark-selection.js":10,"codemirror/mode/gfm/gfm.js":12,"codemirror/mode/xml/xml.js":15}],6:[function(require,module,exports){
